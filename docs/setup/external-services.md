...
!!! danger "「テスト」モードのままだとリフレッシュトークンが 7 日で失効する"
    ...
    **発行から 7 日後に自動的に無効化** されます（テストユーザーも例外ではありません）。
    ...
    毎週再認証が必要になります。
    そのため、運用方針に応じて OAuth 同意画面を「本番環境」に公開するかを検討してください。
    ※この手順は本プロジェクトでは未テストです。
...
!!! warning "本番環境への公開手順は未テスト"
    以下の手順は本プロジェクトでは動作検証していません。
    実施する場合は自己責任で行い、必ず再認証後の動作確認を行ってください。
...
- **対応 A（任意・未テスト）**: OAuth 同意画面を「本番環境」に公開する → [本番環境への公開](#本番環境への公開任意未テスト) を参照
- **対応 B（推奨）**: UI の **「再認証（OAuth）」** ボタンから OAuth フローをやり直す（7 日ごとに繰り返し必要）
