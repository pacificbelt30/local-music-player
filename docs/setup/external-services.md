...
!!! danger "「テスト」モードのままだとリフレッシュトークンが 7 日で失効する"
    OAuth 同意画面のステータスが **「テスト」** の場合、発行されたリフレッシュトークンは
    **発行から 7 日後に自動的に無効化** されます（テストユーザーも例外ではありません）。

    失効すると定期同期が `400 Bad Request` エラーで失敗し続けます。
    毎週再認証が必要になるため、**個人利用でも「本番環境」への公開を推奨します**。
    詳細は後述の [本番環境への公開（推奨）](#本番環境への公開推奨) を参照してください。
...
「テスト」モードの 7 日制限を外すには、OAuth 同意画面を **「本番環境」** に公開します。

!!! tip "審査なしで公開できる（個人利用の場合）"
    Google の審査（OAuth アプリ検証）が必要になるのは「センシティブなスコープを不特定多数に使わせる場合」です。
    **自分だけが使うアプリであれば、審査を通さず本番環境に公開しても問題ありません。**

    本番公開後も、ログイン時に「このアプリは Google で確認されていません」という警告画面が表示されますが、
    「詳細」→「〈アプリ名〉（安全でないページ）に移動」で続行できます。

**手順:**

1. [Google Cloud Console](https://console.cloud.google.com/) → **「APIとサービス」** → **「OAuth 同意画面」**
2. **「アプリを公開」** ボタンをクリック
3. 確認ダイアログで **「確認」**

公開後は再認証すると 7 日制限のない長期有効なリフレッシュトークンが発行されます。
UI の **「再認証（OAuth）」** ボタンから OAuth フローをやり直してください。
...
!!! warning "OAuth 同意画面が「テスト」モードの場合、リフレッシュトークンは 7 日で失効します"
    ...
    失効すると、定期同期タスクが `400 Bad Request` で失敗し、プレイリスト画面の同期カードに
    **「同期エラーが発生しました」** バナーが表示されます（「詳細を見る」でメッセージを確認できます）。

**対処法（いずれか）:**

- **恒久対応（推奨）**: OAuth 同意画面を「本番環境」に公開する → [本番環境への公開](#本番環境への公開推奨) を参照
- **一時対応**: UI の **「再認証（OAuth）」** ボタンから OAuth フローをやり直す（7 日ごとに繰り返し必要）
...
- UI の **「再認証（OAuth）」** ボタンで OAuth フローをやり直すか、
  **「トークンを直接入力」** から [OAuth 2.0 Playground](https://developers.google.com/oauthplayground/) で新しいアクセストークンを取得して再入力してください

!!! warning "OAuth2 Playground のトークンは自動更新されません"
    Playground で取得したリフレッシュトークンは Playground 自身の `client_id` に紐づくため、
    このアプリの `YOUTUBE_CLIENT_SECRET` では更新できません。
    **アクセストークンが切れるたびに手動再入力が必要** になります。
    `YOUTUBE_CLIENT_ID` / `YOUTUBE_CLIENT_SECRET` を設定して「再認証（OAuth）」ボタンを使うことを推奨します。
...
1. Playlists パネルの **「再認証（OAuth）」** をクリック → Google 認可画面へ遷移
2. アカウントを選択して許可すると自動的にアプリへ戻る
3. 同期カードのエラーバナーが消えれば完了

**未接続の場合（初回接続）:**

1. Playlists パネルの **「接続解除」** をクリック（既存の壊れたトークンを削除）
...
| 項目 | 内容 |
|------|------|
| アクセストークン有効期限 | 通常 **1 時間**（期限切れ後は手動再入力が必要） |
| リフレッシュトークン | Playground の `client_id` に紐づくため、アプリでは自動更新**不可** |
| 実用性 | 定期同期が 1 時間ごとに失敗するため、長期運用には不向き |

`YOUTUBE_CLIENT_ID` / `YOUTUBE_CLIENT_SECRET` を設定して「再認証（OAuth）」ボタンを使う方法を推奨します。
